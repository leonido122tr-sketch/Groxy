...
export default function Walls4Calculator({ mode, projectId, initialProject }: Props) {
  ...
  const [widthText, setWidthText] = useState(init ? formatRu(init.width) : '')
  const [lengthText, setLengthText] = useState(init ? formatRu(init.length) : '')

  const [width, setWidth] = useState(init?.width ?? 0)
  const [length, setLength] = useState(init?.length ?? 0)

  const [heightText, setHeightText] = useState(init ? formatRu(init.height) : '')
  const [thicknessText, setThicknessText] = useState(init ? formatRu(init.thickness) : '')
