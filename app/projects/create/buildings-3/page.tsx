...
export default function ProjectSetupPage() {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      if (!projectData || projectData.left <= 0 || projectData.back <= 0 || projectData.right <= 0 || projectData.height <= 0 || projectData.thickness <= 0) {
      ...
      const l1 = Math.max(0, projectData.left + sign * (t / 2))
      const l2 = Math.max(0, projectData.back + sign * t)
      const l3 = Math.max(0, projectData.right + sign * (t / 2))
      ...
      const maxSide = Math.max(projectData.left, projectData.right)
      const floorArea = projectData.principle === 'inside'
        ? Math.max(0, projectData.back * maxSide)
        : Math.max(0, Math.max(0, projectData.back - t) * Math.max(0, maxSide - t / 2))

      const results = { area: floorArea, volume }
      const dims = {
        left: projectData.left,
        back: projectData.back,
        right: projectData.right,
  ...
  return (
    <div className="flex min-h-screen flex-col bg-black font-sans text-white pt-safe">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold leading-tight">Пристрой 3 стены</h2>
