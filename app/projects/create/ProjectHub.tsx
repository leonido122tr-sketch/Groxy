...
import { PdfSaveFloppyButton } from './buildings-2/PdfSaveFloppyButton'
  ...
  return (
    <div className="flex min-h-screen flex-col bg-black font-sans text-white pt-safe">
      <header className="border-b border-white/10">
        <div className="mx-auto max-w-5xl px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <h1 className="text-2xl font-bold truncate max-w-[70%]">{title}</h1>
            <PdfSaveFloppyButton
              variant={projectType}
              onToast={(m) => {
                setToast(m)
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
        ...
        {/* Градиенты через inline style (rgb) — Android WebView не поддерживает oklch/oklab из Tailwind */}
        <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <button
            type="button"
            onClick={() => handleContinue('foundation')}
            className="flex flex-col items-center gap-1.5 rounded-2xl px-5 py-5 text-base font-semibold text-white shadow-md transition-opacity hover:opacity-95"
          ...
          <button
            ...
            className="flex flex-col items-center gap-1.5 rounded-2xl px-5 py-5 text-base font-semibold text-white shadow-md transition-opacity hover:opacity-95"
            style={{ background: 'linear-gradient(to right, rgb(249, 115, 22), rgb(220, 38, 38))' }}
          >
            <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 w-full rounded-xl px-6 py-4 text-base font-semibold transition-colors sm:flex-1 ${
                savedPdfUri ? 'bg-blue-600 text-white hover:bg-blue-500' : 'cursor-not-allowed bg-white/10 text-zinc-500'
              }`}
            >
